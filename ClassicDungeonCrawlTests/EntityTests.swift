...
final class EntityTests: XCTestCase {
    ...
    func test_copy_returnsAnewInstance() {
        let entity = CreatureAppearance(sprite: "Entity", startPosition: .zero)
        let copy = entity.copy()
        XCTAssertTrue(entity !== copy)
    }
    
    func test_copy_returnsAnInstanceWithSamePropertiesAsOriginal() {
        let entity = CreatureAppearance(sprite: UUID().uuidString, startPosition: Vector3D.random)
    ...
    func test_completeCurrentAction_callsActionsComplete() {
        struct BlockAction: Action {
            ...
            static func make(in map: Map, for entity: CreatureAppearance, targetting: Vector3D) -> BlockAction? {
                BlockAction(block: { })
            }
        }
