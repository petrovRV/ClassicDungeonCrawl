...
final class ViewModelTests: XCTestCase {
    ...
    func test_clickTile_selectsEntity_whenATileWithAnEntity_isClicked() throws {
        let entity = CreatureAppearance(sprite: "Example Entity", startPosition: .random)
    ...
    var receivedEntities = [CreatureAppearance?]()
    ...
    func test_clickTile_doesNotChangeSelectedEntity_when_currentAction_isNotNil() {
        let entities  = (0 ..< 10).map { _ in CreatureAppearance(sprite: "\(UUID().uuidString)", startPosition: exampleMap.convertTo3D(exampleMap.tiles.keys.randomElement()!))
        }
    ...
    struct DummyActionWithCoord: Action {
        static func make(in map: Map, for entity: CreatureAppearance, targetting: Vector3D) -> DummyActionWithCoord? {
    ...
    func test_clickTile_setsEntitiesCurrentAction_whenTargettingMode_isTile() throws {
        let entity = CreatureAppearance(sprite: "Test entity", startPosition: .zero)
        let viewModel = ViewModel(map: exampleMap, entities: [entity])
